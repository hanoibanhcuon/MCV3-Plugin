 ...
 */
async function checkPhaseStatus(projectPath, phaseConfig) {
    // Kiểm tra required files
    let completedFiles = 0;
    for (const file of phaseConfig.requiredFiles) {
        if (await (0, file_io_js_1.exists)(path.join(projectPath, file))) {
            completedFiles++;
        }
    }
    ...
    for (const dir of phaseConfig.requiredDirs) {
        if (await (0, file_io_js_1.exists)(path.join(projectPath, dir))) {
            completedDirs++;
        }
    }
 ...
 */
async function buildProjectStatus(projectPath, slug) {
    // Đọc config
...
async function mcStatus(params, projectRoot) {
    ...
    if (!params.projectSlug) {
        ...
        const summaryLines = projects.map(p => `- **${p.name}** (${p.slug}): ${p.currentPhase}, ${p.documentCount} tài liệu`);
        return {
            success: true,
...
//# sourceMappingURL=mc-status.js.map