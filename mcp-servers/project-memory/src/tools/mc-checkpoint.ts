...
interface CheckpointData {
  ...
  /** Tiến độ implementation (chỉ có khi includeProgress: true) */
 ...
 */
async function checkLayerStatus(
  modPath: string,
  modName: string,
  layer: 'controller' | 'service' | 'repository' | 'tests'
): Promise<LayerStatus> {
  ...
  if (!(await exists(filePath))) return 'todo';
...
async function checkMigrationStatus(
  ...
  if (!(await exists(migrationsPath))) return 'todo';
 ...
 */
async function scanImplementationProgress(
  projectRoot: string
  ...
  if (!(await exists(srcPath))) return progress;
 ...
 */
function renderLayerStatus(status: LayerStatus): string {
 ...
 */
function renderProgressSection(progress: ImplementationProgress): string {
  const entries = Object.entries(progress).sort(([a], [b]) => a.localeCompare(b));
  if (entries.length === 0) return '';

  const rows = entries
    .map(([key, p]) =>
      `| \`${key}\` | ${renderLayerStatus(p.controller)} | ${renderLayerStatus(p.service)} | ${renderLayerStatus(p.repository)} | ${renderLayerStatus(p.tests)} | ${renderLayerStatus(p.migration)} | **${p.percentage}%** |`
    )
    .join('\n');
...
export async function mcCheckpoint(
  ...
  try {
    ...
    let implProgress: ImplementationProgress | undefined;
    ...
    const progressSummary = implProgress && Object.keys(implProgress).length > 0
      ? {
          ...
          avgPercentage: Math.round(
            Object.values(implProgress).reduce((s, p) => s + p.percentage, 0) /
