...
import { renderWithDefaults } from '../utils/template-engine.js';
import type {
  McInitParams,
  InitResult,
  ProjectConfig,
 ...
 */
function generateMasterIndex(config: ProjectConfig): string {
  return `# MASTER-INDEX
<!-- ============================================================
 ...
 */
function generateInitialCheckpoint(config: ProjectConfig): string {
 ...
 */
function generateProjectClaudeMd(config: ProjectConfig, slug: string): string {
 ...
 */
function generateMcDataRules(config: ProjectConfig, slug: string): string {
 ...
 */
export async function mcInitProject(
  params: McInitParams,
  projectRoot: string
): Promise<InitResult> {
  // ── Validate input ────────────────────────────────────────────────────
  ...
  try {
    ...
    if (!claudeMdExists) {
      await writeFile(claudeMdPath, generateProjectClaudeMd(config, slug));
    }
    ...
    const mcDataRuleExists = await exists(mcDataRulePath);
    if (!mcDataRuleExists) {
      await writeFile(mcDataRulePath, generateMcDataRules(config, slug));
    }
