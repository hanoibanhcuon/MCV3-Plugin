...
Navigator là skill read-only — không tự gọi `mc_checkpoint`. Tuy nhiên, nếu user chuẩn bị bắt đầu skill tiếp theo (đặc biệt Phase 5+), nhắc nhở:

```
→ "Gợi ý: Dùng mc_checkpoint trước khi chạy {skill-tiếp-theo} để lưu context session hiện tại."
```
...
- KHÔNG tự gọi `mc_checkpoint` (đây là skill read-only, chỉ hiển thị dashboard)
