...
[BẮT BUỘC — RISK-002] Gọi `mc_status` TRƯỚC KHI làm bất cứ điều gì khác:
...
**Nếu mc_status thất bại / lỗi kết nối:** ❌ BLOCKING — DỪNG ngay, báo user kiểm tra MCP Server.
...
**Lỗi: Project config bị hỏng / thiếu _config.json** ❌ BLOCKING
```
...
- `mc_checkpoint` — lưu session-start baseline tự động

## Auto-Checkpoint khi bắt đầu session

[BẮT BUỘC — RISK-002] Sau khi hiển thị dashboard, Navigator **BẮT BUỘC gọi `mc_checkpoint`** để lưu baseline session:

```
mc_checkpoint({
  projectSlug: "<slug>",
  label: "session-start",
  sessionSummary: "Session mới bắt đầu — {project} đang ở {phase}, {N} tài liệu. Bước tiếp theo: {skill-gợi-ý}",
  nextActions: ["Chạy {skill-tiếp-theo}"]
})
```

→ "✅ Session checkpoint đã lưu."

**Lý do:** Lưu baseline đầu session giúp resume chính xác nếu session bị interrupt trước khi skill nào đó hoàn thành. Đây không còn là "gợi ý" mà là bước tự động.
