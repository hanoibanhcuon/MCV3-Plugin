...
2. BẮT BUỘC gọi mc_validate SAU KHI save. Không skip.
   mc_validate({
     filePath: "{SYSTEM}/P1-REQUIREMENTS/URS-{MOD}.md"
   })

   Phân loại kết quả validate (RISK-007):
   - ERROR → ⛔ BLOCKING: DỪNG. Sửa lỗi, save lại, validate lại cho đến khi PASS.
...
3. BẮT BUỘC đăng ký TẤT CẢ IDs vào traceability. Không skip.
   mc_traceability({
     action: "register",
     source: "URS-{MOD}.md",
...
4. BẮT BUỘC gọi mc_checkpoint SAU MỖI module. Không gộp cuối session.
   mc_checkpoint({
...
✅ mc_validate PASS (không còn ERROR nào)
