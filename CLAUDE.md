...
2. **Scale-aware pipeline** — Không cần full 8 phases cho mọi dự án. Dùng scale-decision-matrix để chọn pipeline phù hợp (Micro/Small/Medium/Large/Enterprise). Phases bỏ qua phải được ghi rõ trong PROJECT-OVERVIEW.
...
| domain-expert | agents/domain-expert.md | Phân tích nghiệp vụ (12 industries, gồm Embedded) |
...
| Real Estate | domain-expert/references/industry-realestate.md | BĐS, CRM, commission, Luật Đất đai 2024 |
| Manufacturing | domain-expert/references/industry-manufacturing.md | BOM, MRP, QC, ISO 9001, C/O |
| Education | domain-expert/references/industry-education.md | LMS, quản lý học sinh, Bộ GD&ĐT |
| HR/HRM | domain-expert/references/industry-hr.md | Payroll, BHXH, PIT, Bộ Luật LĐ 2019 |
...
## Batch C — Scale Flexibility & Industry Expansion

Nâng cấp thêm vào MCV3 (sau Sprint 4):

### Scale Decision Matrix

Pipeline không còn one-size-fits-all. Dựa vào quy mô dự án để chọn pipeline phù hợp:

```
Micro  (< 10 endpoints, 1 dev)     → Phase 1 → 5 → 7 (skip 2,3,4,6,8)
Small  (< 30 endpoints, 1-2 devs)  → Phase 1 → 4 → 5 → 7 → 8a
Medium (< 100 endpoints, 2-5 devs) → Phase 1-8 (Phase 3 optional)
Large  (100+ endpoints, 5+ devs)   → Full 8 phases bắt buộc
...
**Education** (LMS, Bộ GD&ĐT),
**HR/HRM** (BHXH, PIT, Bộ Luật Lao động 2019)

### Interview Frameworks mở rộng

Discovery phỏng vấn theo ngành: healthcare, fintech, ecommerce, realestate
(thêm vào fnb, logistics, retail, saas, general đã có)
Reference: `skills/discovery/references/interview-frameworks/`

### Biz-Docs Industry References mở rộng

Phase 3 có KPIs + BRs + compliance per ngành: healthcare, fintech, ecommerce, saas
(thêm vào fnb, logistics, retail đã có)
Reference: `skills/biz-docs/references/industry/`

### Tech Stack Guides mở rộng

Code-Gen giờ có guides cho:
- **Next.js 14+**: App Router, Server Actions, Server Components, Prisma, NextAuth
- **React Native / Flutter**: Expo, EAS, Zustand, TanStack Query, offline-first
